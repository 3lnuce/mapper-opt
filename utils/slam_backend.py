...
from gaussian_splatting.gaussian_renderer import fast_render, fused_render, render
...
class BackEnd(mp.Process):
    ...
    def save_image(
        ...
        test_rgb_pixel_mask = (image.sum(dim=0) > rgb_boundary_threshold).view(
            *mask_shape
        )
    ...
    def initialize_map(self, cur_frame_idx, viewpoint, frame_idx=-1):
        if LOG_ERROR_INIT:
            img_dir = os.path.join(
                self.save_dir, "images", "init_iter_%d" % (self.init_itr_num)
            )
            mkdir_p(img_dir)
        ...
        for mapping_iteration in range(self.init_itr_num):
            ...
            print("loss_init: iter %d, loss: %f" % (mapping_iteration, loss_init))
    ...
    def map(self, current_window, prune=False, iters=1, frame_idx=-1):
        ...
        if LOG_ERROR:
            img_dir = os.path.join(
                self.save_dir,
                "images",
                "frame_%d_iter_%d_cam_%d" % (frame_idx, iters, len(current_window)),
            )
            mkdir_p(img_dir)
        ...
        python_timer = open("python_timer.txt", "a+")
        ...
        for iter_idx in range(iters):
            ...
            print(
                ...
                iter_idx,
                ", gaussians: ",
                self.gaussians._xyz.shape,
                ", flag: ",
                self.gaussians.is_active.shape,
            )
            tic.record()
            self.iteration_count += 1
            self.last_sent += 1
            ...
            for cam_idx in range(len(current_window)):
                ...
                render_info = "######frame_%d_cam_%d_iter_%d.log" % (
                    frame_idx,
                    cam_idx,
                    iter_idx,
                )
            ...
            torch.cuda.synchronize()
            python_timer.write(
                "loss_backward: " + str(tic_loop.elapsed_time(toc_loop)) + "\n"
            )

            # print(render_info)
            # print("Backend [Mapping] Loss.backward: ", tic.elapsed_time(toc))

            # for param in self.gaussians.optimizer.param_groups:
            #     param_tensor = param['params'][0]
            #     if ((param['name'] == 'xyz')):
            #         # print (param['name'])
            #         print ("Sanity check for reference grad after loss_mapping.backward: ", param_tensor.grad)
            ...
            #     if ((param['name'] == 'xyz')):
            #         # print (param['name'])
            #         # print ("grad after loss_mapping.backward: ", param_tensor.grad)
            #         print ("Sanity check for fused grad after loss_mapping.backward: ", param_tensor.grad)
        ...
        while True:
            if self.backend_queue.empty():
                ...
                if data[0] == "stop":
                    ...
                    self.initialize_map(
                        cur_frame_idx, viewpoint, frame_idx=cur_frame_idx
                    )
                    ...
                    self.add_next_kf(
                        ...
                        viewpoint,
                        depth_map=depth_map,
                        last_viewport=self.viewpoints[last_vp_key],
                    )
                    ...
                    self.map(
                        self.current_window, iters=iter_per_kf, frame_idx=cur_frame_idx
                    )
